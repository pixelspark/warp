...
extension QBERethinkSourceStep: QBEFormConfigurableStep {
	...
	var form: Form {
		...
		form +++ Section("Server".localized)
			<<< TextRow("host") { row in
				...
				row.value = self.server
				row.placeholder = "localhost".localized
				row.onChange { self.server = $0.value ?? "localhost" }
		...
		form +++ Section("")
			<<< PushRow<QBESecret>() { pr in
				pr.options = QBESecret.secretsForService("rethink")
				pr.noValueDisplayText = "Recent servers".localized

				pr.onChange { pr in
					if let secret = pr.value {
						self.username = secret.accountName
						self.server = secret.url?.host ?? self.server
						self.port = secret.url?.port ?? self.port

						pr.value = nil
						form.setValues([
							"host": secret.url?.host ?? self.server,
