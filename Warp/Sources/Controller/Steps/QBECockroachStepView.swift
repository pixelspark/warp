...
internal class QBECockroachSourceStepView: QBEConfigurableStepViewControllerFor<QBECockroachSourceStep>, QBEAlterTableViewDelegate {
	...
	let serviceDatasetSource = QBESecretsDataSource(serviceType: "cockroach")
	...
	@IBAction func createTable(_ sender: NSObject) {
		if let warehouse = step.warehouse {
			...
			vc.delegate = self
			vc.warehouseName = String(format: NSLocalizedString("CockroachDB database '%@'", comment: ""), self.step.databaseName)
